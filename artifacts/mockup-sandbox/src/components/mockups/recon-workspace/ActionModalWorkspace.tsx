...
import {
  ChevronRight, ChevronDown, MoreHorizontal, FileDown, ArrowLeft, ArrowRight,
  AlertTriangle, XCircle, CheckCircle2, Flag, Search, TrendingUp, TrendingDown,
  Calculator, Pencil, Check, X, Gavel, FileWarning, Sparkles, Eye
} from "lucide-react";
  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen bg-background font-sans">
                ...
                {MOCK_SUMMARY.map((row) => {
                  ...
                  return (
                    <TableRow
                      ...
                      onClick={() => isExpandable && openReason(row.reason)}
                    >
                      <TableCell className="py-2 pl-4">
                        <SeverityBorder usd={row.discUsd} />
                        <span className={`text-sm font-medium flex items-center gap-1.5 ${
                          row.type === "special-ar" ? "text-amber-700" : row.type === "special-cancel" ? "text-red-600" : row.type === "reconciled" ? "text-green-600" : "text-foreground"
                        }`}>
        ...
        <Dialog open={isOpen} onOpenChange={(open) => { if (!open) { setModalView({ level: "closed" }); setActionFeedback(null); } }}>
          <DialogContent className="max-w-[90vw] max-h-[88vh] flex flex-col p-0 gap-0">
            {actionFeedback && (
            ...
            <div className="px-6 pt-5 pb-4 border-b flex-shrink-0">
              <div className="flex items-center gap-2 mb-1">
                {(modalView.level !== "reason" && modalView.level !== "closed") && (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      else setModalView({ level: "reason", reason: currentReason });
                    }}
                  >
                ...
                <nav className="flex items-center gap-1.5 text-sm flex-wrap">
                  <button
                    className={`font-medium ${modalView.level !== "reason" && modalView.level !== "reason-spnet" ? "text-primary hover:underline cursor-pointer" : "text-foreground"}`}
                  ...
                  {currentTid && (
                    <>
                      ...
                      <button
                        className={`font-mono font-medium ${modalView.level === "bookings" ? "text-primary hover:underline cursor-pointer" : "text-foreground"}`}
                        onClick={() => { if (modalView.level === "bookings") setModalView({ level: "tid", reason: currentReason, tid: currentTid, experience: currentExperience }); }}
                      >
                        {currentTid}
                      </button>
              ...
              <div className="flex items-center justify-between mt-3">
                <div className="flex items-center gap-2">
                  ...
                  {tidData && currentTid && (
                    <>
                      <Badge variant="outline" className="text-xs font-mono">{tidData.fm}</Badge>
                      <Badge variant="secondary" className="text-xs">{tidData.bidCount} bookings</Badge>
                    </>
                  )}
                </div>
            ...
            <div className="flex-1 overflow-auto">
              ...
              {/* ──── REASON LEVEL: TID list + bulk actions ──── */}
              {modalView.level === "reason" && (
                <div className="p-4 space-y-3">
                  <div className="flex items-center gap-2 px-1">
                    <Sparkles className="h-4 w-4 text-violet-500" />
                    <span className="text-xs text-muted-foreground">
                      Consistent margin gap: HO expected 18.2% take rate but actual is 12.8% — a 5.4pp shortfall across 4 TIDs. Likely a rate configuration issue.
                    </span>
                  </div>
                          ...
                          return (
                            <TableRow key={tid.tid} className="h-10 cursor-pointer hover:bg-muted/40" onClick={() => openTid(tid)}>
                              ...
                              <TableCell className="py-2 text-right font-mono text-sm text-red-600">
                                {fmt(tid.discLc)} <span className="text-[10px] text-muted-foreground font-sans">({pct}%)</span>
                              </TableCell>
              ...
              {modalView.level === "reason-spnet" && (
                <div className="p-4 space-y-3">
                  <div className="rounded-md border overflow-hidden">
                    <div className="px-4 py-3 border-b bg-blue-50">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-medium">Confirm: Set all {reasonData?.count} bookings to SP Net</div>
                          <div className="text-xs text-muted-foreground">Across {MOCK_TIDS.length} TIDs</div>
                        </div>
                      </div>
                    </div>
                    <div className="p-4 space-y-3">
                      <div className="grid grid-cols-3 gap-3">
                        <div className="rounded-md border p-3 bg-blue-50/50">
                          ...
                          <div className="text-lg font-mono font-semibold text-blue-700">{fmt(MOCK_TIDS.reduce((s, t) => s + t.spNet, 0))} EUR</div>
                        </div>
                        <div className="rounded-md border p-3 bg-green-50/50">
                          <div className="text-xs text-muted-foreground mb-1">HO Net Total</div>
                          <div className="text-lg font-mono font-semibold text-green-700">{fmt(MOCK_TIDS.reduce((s, t) => s + t.hoNet, 0))} EUR</div>
                        </div>
                        <div className="rounded-md border p-3 bg-muted/30">
                          <div className="text-xs text-muted-foreground mb-1">Difference (SP − HO)</div>
                          <div className="text-lg font-mono font-semibold text-amber-600">+{fmt(MOCK_TIDS.reduce((s, t) => s + t.spNet - t.hoNet, 0))} EUR</div>
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            Paying SP Net now. The difference of <span className="font-mono font-semibold text-amber-600">{fmt(Math.abs(MOCK_TIDS.reduce((s, t) => s + t.spNet - t.hoNet, 0)))} EUR</span> will be tracked as a dispute.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between px-4 py-3 gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">This is HO error — to be checked with internal teams</div>
                        </div>
                      </div>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <Button variant="ghost" size="sm" onClick={() => setModalView({ level: "reason", reason: currentReason })}>
              ...
              {modalView.level === "tid" && tidData && (
                <div className="p-4 space-y-3">
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div className="rounded-md border p-2.5 bg-blue-50/50">
                      <span className="text-muted-foreground">SP Net Total:</span>{" "}
                      <span className="font-mono font-semibold text-blue-700">{fmt(tidData.spNet)} EUR</span>
                    </div>
                    <div className="rounded-md border p-2.5 bg-green-50/50">
                      <span className="text-muted-foreground">HO Net Total:</span>{" "}
                      <span className="font-mono font-semibold text-green-700">{fmt(tidData.hoNet)} EUR</span>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <div className="rounded-md border overflow-hidden cursor-pointer hover:bg-blue-50/60 transition-colors"
                      onClick={() => setModalView({ level: "tid-spnet", reason: currentReason, tid: currentTid, experience: currentExperience })}>
                      <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="h-8 w-8 rounded-md bg-blue-100 flex items-center justify-center"><TrendingUp className="h-4 w-4 text-blue-600" /></div>
                          <div>
                            <div className="text-sm font-medium">Update to SP Net</div>
                            <div className="text-xs text-muted-foreground">Set Amount Payable = SP Net for all {tidData.bidCount} bookings ({fmt(tidData.spNet)} EUR)</div>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">Set Amount Payable = HO Net for all {tidData.bidCount} bookings ({fmt(tidData.hoNet)} EUR)</div>
                          </div>
                        </div>
                        <ArrowRight className="h-4 w-4 text-muted-foreground" />
                      </div>
                    ...
                    <div className="rounded-md border overflow-hidden cursor-pointer hover:bg-violet-50/60 transition-colors"
                      onClick={() => setModalView({ level: "tid-pax", reason: currentReason, tid: currentTid, experience: currentExperience })}>
                      <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">Enter final unit price per pax type to recalculate</div>
                          </div>
                        </div>
                        <ArrowRight className="h-4 w-4 text-muted-foreground" />
                      </div>
                    ...
                    <div className="rounded-md border overflow-hidden cursor-pointer hover:bg-muted/40 transition-colors"
                      onClick={() => setModalView({ level: "bookings", reason: currentReason, tid: currentTid, experience: currentExperience })}>
                      <div className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-sm font-medium">View & Edit Individual Bookings</div>
                            <div className="text-xs text-muted-foreground">Set net, dispute, or flag issues per booking</div>
                          </div>
                        </div>
                        <ArrowRight className="h-4 w-4 text-muted-foreground" />
                      </div>
              ...
              {modalView.level === "tid-spnet" && tidData && (
                <div className="p-4 space-y-3">
                  ...
                  <div className="flex justify-between pt-1">
                    <Button variant="ghost" size="sm" onClick={() => setModalView({ level: "tid", reason: currentReason, tid: currentTid, experience: currentExperience })}><ArrowLeft className="h-3.5 w-3.5 mr-1.5" /> Back</Button>
