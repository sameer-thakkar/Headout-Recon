...
- **Export Formatting**: Indian number format, DD/MM/YYYY date format, specific sorting, table styling, and auto column widths for Excel and Google Sheets. Export is split into two separate files: Reconciliation Analysis (Discrepancy Analysis, Draft Messages, DRI sheets) and Financial Report (Payable Summary, SP Invoice, HO Report Updated). Both Excel and Google Sheets support this split via `/api/runs/:runId/export/analysis`, `/api/runs/:runId/export/financial`, `/api/runs/:runId/export-gsheet/analysis`, `/api/runs/:runId/export-gsheet/financial`. Export logic lives in `server/export-routes.ts` with shared utilities in `server/export-utils.ts`.
