...
- **Negative SP Net Handling**: Bookings with negative SP Net are handled in two cases: (1) CANCELLED + HO Net = 0 → auto-reconciled as "Cancelled-Refund OK" with Final Net Price = 0, (2) all other negatives → "Negative SP - Partial Refund" discrepancy reason. UI shows red-tinted rows with "Refund" badge and bold red SP Net values. In Purchase Reconciliation, negative values are called out as refunds in the Balance & Deposits section. Auto-FNP computation for negative SP bookings: (a) HO=0 → FNP=0, (b) |SP|=|HO| → FNP=0, (c) otherwise → FNP=||HO|-|SP||. Sub-classification badges (Zero HO / Matched / Difference) shown per booking. Summary card in reason group shows counts per sub-type + totals. Verification checkbox required before Apply & Confirm; warning banner shown when unverified.
...
- **Portal Reloads Upload**: Separate file upload section on the home page for reload data. Parses "Finance Zendesk Tickets Portal Partner ID", "Finance Zendesk Tickets Paid Amount", Zendesk ID, Date of Payment, and Amount Loaded at Date columns. Sums paid amounts per BE ID, and uses the total as the Reloads value in Purchase Reconciliation (overriding the vendor balance reloads when portal reload data exists). API: `/api/portal-reloads`, `/api/portal-reloads/:beId`, `/api/portal-reloads/upload`, `/api/portal-reloads/save`. DB table: `portal_reloads`.
