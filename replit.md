...
- `/api/runs/:runId/actioning-progress`: GET - Returns TID actioning progress per-reason and overall. Structure: `{ overall: { total, actioned }, byReason: { [reason]: { total, actioned } } }`. A TID is "actioned" if any of its bookings have a price override or dispute. Used by the global progress bar and per-discrepancy progress indicators in the summary table.
