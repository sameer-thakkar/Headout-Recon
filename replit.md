...
- `/api/issues`: POST for creating issues, GET `/api/issues/:runId` for listing, PATCH `/api/issues/:issueId` for inline editing, DELETE `/api/issues/:issueId` for removal. Issue Tracker is built at DRI-Discrepancy level with columns: Date, Payment Method, Period, Assignee, BE ID/Name, Currency, Discrepancy LC/USD, DRI Team (auto-detected with manual override), Error Bucket (dropdown populated from reconciliation `reasonCodes` + "Other"), RCA (dependent dropdown filtered by Error Bucket via `errorBucketRcaMapping` in shared/schema.ts), Slack Link, Workings Link, and Issue Status dropdown. Error Bucket auto-populates with the discrepancy reason when issues are created. All editable fields support inline editing via PATCH endpoint with server-side validation for Error Bucket/RCA pairing.
